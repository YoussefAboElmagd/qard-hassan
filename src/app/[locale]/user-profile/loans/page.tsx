...
export default function LoanDashboard() {
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm">
      ...
      {loans.map((loan: Loan) => (
        <LoanCard key={loan.id} id={loan.id} name={loan.name} loan_amount={loan.loan_amount} loan_reason={loan.loan_reason} number_of_installments={loan.number_of_installments} submission_date={loan.submission_date} start_date={loan.start_date} end_date={loan.end_date} status={loan.status} />
        ))}
      </div>
    </div>
  );
